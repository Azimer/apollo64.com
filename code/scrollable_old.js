...
function Scrollable(obj,url) {
  ...
  "<layer name=scrollable >"+
   "<layer z-index=1 name=h width="+swidth+" visibility=hide>"+
        '<layer name=left top=0 left=0 bgcolor=black visibility=hide>'+
         '<img name=left src='+Scrollable.basePath+Scrollable.upLeftIcon+'>'+
        '</layer>'+
        '<layer name=right top=0 bgcolor=black visibility=hide>'+
         '<img name=right src='+Scrollable.basePath+Scrollable.upRightIcon+'>'+
        '</layer>'+
        '<layer name=empty top=0 left=16 background="'+Scrollable.basePath+Scrollable.barBackground+'" visibility=hide>'+
   ...
   "<layer z-index=1 name=v height="+sheight+" width=16 visibility=hide>"+
    '<layer name=up width=16 top=0 left=0 bgcolor=black>'+
        ...
        '</layer>'+
   "</layer> "+
   '<layer z-index=1 name=resizer visibility=hide></layer>';
  if (obj.constructor!=Layer)
   s+='<layer visibility=hide z-index=0 name=main src='+url+' width='+(swidth-30)+' clip='+swidth+','+sheight+'></layer>';
   s+=
  '</layer>';
  document.write(s);
